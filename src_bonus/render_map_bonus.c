...
/*   Updated: 2025/02/17 00:06:41 by sel-abbo         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	loading_textures(t_game *game)
{
	...
	textures_collect(game);
	textures_enemy(game);
	if (!game->wall || !game->player || !game->collect[0] || !game->exit
		|| !game->empty || !game->r_player || !game->l_player || !game->b_player
		|| !game->nc_exit || !game->enemy[0] || !game->enemy[1]
		|| !game->enemy[2])
		...
		while (game->map[y][++x])
		{
			if (game->map[y][x] == '1')
				...
				mlx_put_image_to_window(game->mlx, game->win,
					game->collect[game->col_frames], x * 32, y * 32);
			else if (game->map[y][x] == 'D')
				mlx_put_image_to_window(game->mlx, game->win,
					game->enemy[game->enemy_frames], x * 32, y * 32);
